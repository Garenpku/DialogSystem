...
class DecoderRNN(BaseRNN):
    ...
    def forward_step(self, input_var, hidden, encoder_outputs, function, mix=None, score_copy=None, use_copy=False,
        ...
        if use_copy:
            decoder_input = torch.cat([mix, embedded, concept_rep], dim=-1)
        else:
        ...
        score_vocab = torch.exp(score_vocab)
        if use_copy:
            final_score = score_vocab + score_copy
        else:
    ...
    def copy(self, context, decoder_state, dialog_state, concepts, embeddings, tgt_vocab, cpt_vocab):
        # batch * hidden/dialog
            ...
            for di in range(max_length):
                ...
                if di == 0:
                    ...
                    if torch.cuda.is_available():
                        mix = mix.cuda()
                if use_copy:
                    #score_copy = self.copy(context, decoder_hidden.squeeze(), batch_state, batch_concepts,
                    #                       batch_embeddings, tgt_vocab, cpt_vocab)
                    score_copy = torch.zeros([len(context), self.output_size])
                    if torch.cuda.is_available():
                        score_copy = score_copy.cuda()
